...
static const char
    *kMonthsAbbrev[12] = {"Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"};
...
#   endif  // ELPP_COMPILER_MSVC || defined(_MSC_EXTENSIONS)
    ...
    getTimeDifference(const struct timeval &endTime, const struct timeval &startTime, base::TimestampUnit timestampUnit)
    {
        if (timestampUnit == base::TimestampUnit::Microsecond) {
            ...
            return static_cast<unsigned long long>(
                (((endTime.tv_sec - startTime.tv_sec) * 1000000) + (endTime.tv_usec - startTime.tv_usec)) / 1000);
        }
    }
        ...
        for (; *format; ++format) {
            if (*format == base::consts::kFormatSpecifierChar) {
                switch (*++format) {
                    ...
                    case 'F':  // AM/PM
                        buf =
                            base::utils::Str::addToBuff((tInfo->tm_hour >= 12) ? base::consts::kPm : base::consts::kAm,
                                                        buf,
                                                        bufLim);
                        continue;
                    default:
...
protected:
    ...
    virtual void updateDateFormat(std::size_t index, base::type::string_t &currFormat) ELPP_FINAL
    {
        ...
        if ((currFormat.size() > index) && (ptr[0] == '{')) {
            // Player has provided format for date/time
            ++ptr;
...
public:
    ...
    void setRemainingToDefault(void)
    {
        ...
        unsafeSetIfNotExist(Level::Trace,
                            ConfigurationType::Format,
...
private:
    ...
    void insertFile(Level level, const std::string &fullFilename)
    {
        ...
        std::string
            filePath = base::utils::File::extractPathFromFilename(resolvedFilename, base::consts::kFilePathSeperator);
        ...
        auto create = [&](Level level)
        {
            ...
            if (filestreamIter == m_logStreamsReference->end()) {
                ...
                m_logStreamsReference
                    ->insert(std::make_pair(resolvedFilename, base::FileStreamPtr(m_fileStreamMap.at(level))));
            }
            else {
...
public:
        ...
        m_preRollOutCallback(base::defaultPreRollOutCallback)
    {
        ...
        performanceLogger->configurations()
            ->setGlobally(ConfigurationType::Format, std::string("%datetime %level %msg"));
...
#endif  // ELPP_ASYNC_LOGGING
        installPerformanceTrackingCallback<base::DefaultPerformanceTrackingCallback>(std::string(
...
private:
    ...
    void dispatch(base::type::string_t &&logLine)
    {
        if (m_data->dispatchAction() == base::DispatchAction::NormalLog) {
                ...
                if (fs != nullptr) {
                    ...
                    if (fs->fail()) {
                        ELPP_INTERNAL_ERROR("Unable to write log to file ["
                                                << m_data->logMessage()->logger()->m_typedConfigurations
                                                    ->filename(m_data->logMessage()->level()) << "].\n"
            ...
            if (m_data->logMessage()->logger()->m_typedConfigurations
                ->toStandardOutput(m_data->logMessage()->level())) {
                if (ELPP->hasFlag(LoggingFlag::ColoredTerminalOutput))
                    m_data->logMessage()->logger()->logBuilder()
                        ->convertToColoredOutput(&logLine, m_data->logMessage()->level());
    ...
    base::type::string_t build(const LogMessage *logMessage, bool appendNewLine) const
    {
        ...
        if (logFormat->hasFlag(base::FormatFlags::DateTime)) {
            ...
            base::utils::Str::replaceFirstWithEscape(logLine, base::consts::kDateTimeFormatSpecifier,
                                                     base::utils::DateTime::getDateTime(logFormat->dateTimeFormat()
                                                                                            .c_str(),
...
public:
                    ...
                    const char *func = "")
    {
        ...
        if (m_enabled) {
            ...
            base::utils::DateTime::gettimeofday(&m_endTime);
            base::type::string_t
                formattedTime = m_hasChecked ? getFormattedTimeTaken(m_lastCheckpointTime) : ELPP_LITERAL("");
...
protected:
    void handle(const PerformanceTrackingData *data)
    {
        ...
        el::base::Writer(m_data->performanceTracker()->level(), m_data->file(), m_data->line(), m_data->func())
            .construct(1, m_data->loggerId().c_str()) << ss.str();
    }
private:
